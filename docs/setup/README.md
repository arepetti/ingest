...
| [powerbi/](powerbi/README.md)         | Connecting Power BI (or any OData v4 client) to the OData feeds (`/odata/samples` data + `/odata/scorecard` RAG board). Custom-header auth recipe, query options, per-feed column references, data-model tips, refresh schedules. |
