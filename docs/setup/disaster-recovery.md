...
1. **Stop the bleeding.** If it's one bad actor, disable the offending service account / API key. If the problem is more widespread (a faulty integration, a bad bulk-import file, uncertainty about the blast radius), use the in-app kill switch instead of taking the app down: **Settings → Ingestion → Close all submissions** (optionally with a message shown to users) immediately blocks every service-facing write (API/bulk import/Teams) with a 503 while leaving reads, OData, and admin remediation available — see [admin-user-guide/settings.md § Ingestion](../admin-user-guide/settings.md#ingestion). It's faster and less disruptive than scaling the app to zero (`az containerapp update --name $App --resource-group $Rg --min-replicas 0`), which remains the fallback when you need to stop *everything*, including admin access.
