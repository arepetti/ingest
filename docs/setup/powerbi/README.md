...
There are two feeds today. Pick by what you want to do — most reports use **samples**; reach for **scorecard** when you only need the at-a-glance status.
...
> **Planned:** a simplified **schemas** metadata feed at `/odata/schemas` (schema/value labels, units, types — no versioning/notes/layout). Until it ships, pull that metadata from the JSON endpoint `/api/admin/schemas` as a second Web query and join on names — see [samples.md → joining schema metadata](samples.md#my-report-needs-joins-to-schema-metadata). This page will be updated when the feed lands.
...
Every `/odata` feed is gated by the **`query:read`** capability. In practice that means an account whose role is **Operator** or **Admin** (both carry `query:read` by default); a **Service**-role key cannot read it. If you use [custom capabilities](../../admin-user-guide/accounts.md), the exact gate is the `query:read` capability rather than the role name.
